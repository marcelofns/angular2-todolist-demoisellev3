...
export class UsuarioService {
  ...
  list(currentPage: number, itemsPerPage: number) {
    ...
    return this.http.get('~main/users')
      .map(
      res => res.json()
  ...
  create(usuario: Usuario) {
    return this.http.post('~main/users', usuario);
  }

  update(usuario: Usuario) {
    return this.http.put('~main/users/' + usuario.id, usuario);
  }

  delete(usuario: Usuario) {
    return this.http.delete('~main/users/' + usuario.id);
  }
}
